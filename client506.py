...
class MyDiscourseClient(DiscourseClient):
    ...
    def _do_request(self, verb, path, params={}, files={}, data={}, json={}):
        """
    ...
    def _request(self, verb, path, params={}, files={}, data={}, json={}):
        ...
        if max_attempts == 1:
            # Appropriate for webhook, for example
            return DiscourseClient._request(self, verb, path, params, files, data, json)
        else:
            # some stuff I added to handle crawling
            for attempt in range(max_attempts):
                try:
                    return self._do_request(verb, path, params, files, data, json)
                except DiscourseClientError, exc:
                    ##print '%s failed; trying again' % path
                    if not 'post' in path:
                        # this is expected to fail when querying for post that are not known to exist yet
                        # This is also the exception when Discourse throttles
                        logger.info('DiscourseClientError %s, %s' % (exc, path))
                        if max_attempts > 1:
                            sleep(30)
                except requests.ReadTimeout, exc:
                    logger.warning('requests.ReadTimeout- %s, %s, %s' % (exc, path, str(params)))
                    sleep(30)
                except requests.ConnectionError, exc:
                    logger.warning('requests.ConnectionError- %s' % exc)
                    sleep(30)
